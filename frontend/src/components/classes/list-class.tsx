...
import ListClassFilterCard from './list-class-filter-card'
...
export default function ListClass ({ userRole }: { userRole: User['role'] }) {
      ...
      {userRole === 'teacher'
        ...
        {clases.map((alumno) => (
          ...
          >
            ...
            <div className={`flex flex-1 items-center justify-between bg-card rounded-md cursor-pointer ${selectedId === alumno.id ? 'hidden' : ''}`}>
